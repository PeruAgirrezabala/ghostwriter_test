<?php
    ...
    if(isset($button)) {
		...
		if ($user_password != $user_password2) {
			...
			try
			{ 
				$stmt = $db_con->prepare("(SELECT id FROM tools_users WHERE user_email='".$email."') UNION (SELECT id FROM tools_clientes where user_email='".$email."')");
				...
				if($count == 0){
					$db_con->query("INSERT INTO tools_clientes 
									...
									VALUES ('".$email."','".$password."', '".$_POST['signname']."', '".$_POST['sign_apellidos']."', '".$_POST['signempresa']."', '".$_POST['signtlfno']."')");
                                        ...
                                        //$mail->isHTML(true); 
                                        $mail->Subject  = 'CRM WEROI // Nueva solicitud de acceso';
                                        $mail->Body     = 'Tienes una nueva solicitud de acceso a CRM WEROI por parte de un Cliente:'. $salto.$salto .
                                                     'Sus datos son los siguientes:'. $salto .
                                                     'Nombre: '.$_POST['signname']. $salto .
                                                     'Apellidos: '.$_POST['sign_apellidos']. $salto .
                                                     'Telefono: '.$_POST['signtlfno']. $salto .
                                                     'Empresa: '.$_POST['signempresa']. $salto .
                                                     'Email: '.$email. $salto.$salto .
                                                     'Haz click en el siguiente enlace para dar acceso a este Cliente: '.$email.
                                                     ' / https://tools.we-roi.com/accesos.php?p=c';
                                        if(!$mail->send()) {
