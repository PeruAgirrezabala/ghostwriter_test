...
<script>
	...
	$(document).ready(function() {
            ...
            $('#menuitem-plantillas').addClass('active');
            
            loadSelect("newplantilla_tipo","TIPOS_DOCISO","id","","");
            
            $("#docplan_docfechaexp").focusout(function() {
                $("#uploaddocsPLAN").fileinput('destroy');
                $("#uploaddocsPLAN").fileinput({
                    uploadUrl: "upload.php?iddoc=" + $("#docPLAN_iddoc").val() + "&fecha=" + $("#docplan_docfechaexp").val(),
            ...
            $("#tabla-plantillas tr > td:not(:nth-child(6)):not(:nth-child(7))").click(function() {
                loadPlanInfo($(this).parent("tr").data("id"));
                //loadOfertaDetalleInfo($(this).data("id"));
                $("#addPlantilla_model").modal('show');
            });
            
            $("#btn_del_plantilla_model").click(function() {
                $("#confirmar_delete_plantilla_model").modal("show");
            ...
            $("#btn_del_plantilla").click(function() {
                ...
                $.ajax({
                    ...
                    url: "savePlantilla.php",  
                    data: {
                        newplantilla_delplan : $("#newplantilla_delplan").val()
                    },
                    dataType: "text",       
                    success: function(response)  
                    {
...
<title>PLANTILLAS | Erp GENELEK</title>
</head>
...
<body>
    ...
    <section id="contenido">
        <div id="erp-titulo" class="one-column">
            <h3>
                REGISTROS / PLANTILLAS Y EJEMPLOS
            </h3>
        </div>
        <div id="dash-content">
            <div id="dash-proyectosactivos" class="one-column">
                <h4 class="dash-title">
                    ...
                    <? 
                        include($pathraiz."/apps/plantillas/includes/tools-plan.php"); 
                    ?>
                </h4>
                <div id="dash-empresas" style="padding:10px;">
                    <? 
                        include($pathraiz."/apps/plantillas/vistas/tabla-plantillas.php"); 
                    ?>
                </div>
            </div>
