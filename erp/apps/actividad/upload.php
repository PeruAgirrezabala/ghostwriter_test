<?
    ...
    //if ($_GET['actpath'] == "") {
        ...
        $sql = "SELECT fecha, nombre, ref, path FROM ACTIVIDAD WHERE id = ".$_GET['id_act'];
        $result = mysqli_query($connString, $sql) or die("Error al seleccionar el numero de Actividad");
        $registros = mysqli_fetch_row ($result);
        $fechaAct = $registros[0];
        $tituloAct = $registros[1];
        $path = $registros[3];
        //$REF = str_replace("/","-",$registros[2]);
        //$nombre = str_replace("/","-",str_replace(" ", "_", $tituloAct));        
        //$path = "/".date('Y', strtotime($fechaAct))."/".$REF."_".$nombre;
        //$pathYear = "/".date('Y', strtotime($fechaAct))."/";
    //}
    //else {
        //$path = $_GET['actpath'];
    //}
