...
<table class="table table-hover" id="tabla-plantillas">
    <thead>
        <tr>
            ...
            <th class="text-center">ÚLTIMA VERSION</th>
    ...
    <?
        ...
        $sql = "SELECT 
                    ...
                    PLANTILLAS.ref,
                    (SELECT doc_path FROM PLANTILLAS_VERSIONES WHERE plantilla_id = plan ORDER BY fecha_exp DESC, id DESC LIMIT 1) as path,
                    (SELECT fecha_exp FROM PLANTILLAS_VERSIONES WHERE plantilla_id = plan ORDER BY fecha_exp DESC, id DESC LIMIT 1) as fecha_exp,
                    (SELECT id FROM PLANTILLAS_VERSIONES WHERE plantilla_id = plan ORDER BY fecha_exp DESC, id DESC LIMIT 1) as ver_id  
                FROM 
                    PLANTILLAS 
                WHERE tipo = 2 
                ORDER BY 
        ...
        while ($registros = mysqli_fetch_array($resultado)) {
            ...
            echo "
                <tr data-id='".$id."' class='licencia'>
                    ...
                    <td class='text-center'><button class='btn-default upload-doc-PLAN' data-id='".$id."' title='Subir Documento'><img src='/erp/img/upload.png' style='height: 15px;'></button></td>
                </tr>
            ";
        }   
    ?>
