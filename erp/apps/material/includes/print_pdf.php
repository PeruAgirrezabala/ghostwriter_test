<?php
    ...
    $rowTot = mysqli_num_rows ($resultado);
    ...
    while ($registros = mysqli_fetch_array($resultado)) {
        ...
        /* CAMBIO
        if ($registros[25] == "" || $registros[25] == null) {
            $ref = $registros[1];
        }
        else {
            $ref = $registros[25];
        ...
        */
        ...
        if($contador==0){
            ...
            // $rowTot
            // $contador==16
            // CAMBIOS DEPENDIENDO DEL ALTO DE LA LINEA......
        }elseif(is_int($contador/11)){
            $html .= "
                </tbody>
                </table>
    ...
    //file_put_contents("log_0.txt", "kkk");
    ...
    //file_put_contents("log0.txt", "aaa");
    // 20/05/2022 Para que se actualiza el pedido?
    ...
    //file_put_contents("log1.txt", "bbb");
    ...
    //file_put_contents("log2.txt", "Pre-conexion");
// connection to ftp
    $ftp_connection = ftp_connect($ftp_server);
    $connection_result = ftp_login($ftp_connection, $ftp_username, $ftp_password);
    //file_put_contents("log3.txt", "Post-conexion");
