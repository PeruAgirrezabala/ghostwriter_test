<?
    ...
    if (empty($_FILES['newensayo_adjunto'])) {
    ...
    $sql = "SELECT entrega_id, nombre FROM ENSAYOS WHERE id = ".$_GET["ensayoid"];
    file_put_contents("LOG13-0.txt", $_GET['nombre']);
    $result = mysqli_query($connString, $sql) or die("Error al seleccionar la entregaid");
    $registros = mysqli_fetch_row ($result);
    
    $nombreDOC = str_replace("/","-",str_replace(" ", "_", $registros[1])); 
    
    $sql = "SELECT path FROM ENTREGAS WHERE id = ".$registros[0];
